...
import { Home, Compass, MessageCircle, Gift, User, BarChart3, Globe, Sparkles } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 lg:pl-72">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-1 rounded-full border px-3 py-1.5 text-xs font-medium hover:bg-muted transition"
            >
      ...
      <div className="mx-auto flex max-w-6xl">
        {/* Desktop sidebar */}
        <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-64 flex-col border-r bg-card pt-20 px-4">
          <nav className="flex flex-col gap-1">
            {tabs.map((t) => {
              const active = path.startsWith(t.to);
              const Icon = t.icon;
              return (
                <Link key={t.to} to={t.to} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "bg-primary/10 text-primary font-semibold" : "text-muted-foreground hover:bg-muted"}`}>
                  <Icon className="h-4 w-4" />
                  {lang === "ko" ? t.label : t.labelEn}
                </Link>
              );
            })}
            <Link to="/admin" className={`mt-4 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${path.startsWith("/admin") ? "bg-secondary/10 text-secondary font-semibold" : "text-muted-foreground hover:bg-muted"}`}>
        ...
        <main className="flex-1 lg:ml-64 pb-24 lg:pb-12 min-h-[calc(100vh-4rem)]">
          {children}
        </main>
      </div>
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 border-t bg-background/95 backdrop-blur-md">
        <div className="grid grid-cols-5">
          {tabs.map((t) => {
            ...
            const Icon = t.icon;
            return (
              <Link key={t.to} to={t.to} className="flex flex-col items-center justify-center gap-1 py-2.5">
      ...
      <button
        ...
        className={`lg:hidden fixed bottom-20 right-4 z-40 flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-semibold shadow-lg transition ${demo ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
      >
        🎭 데모 모드 {demo ? "ON" : "OFF"}
      </button>
